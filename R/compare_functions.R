...
summary.comp.funcs.res.dt <- function(comp_funcs_res){
  for(index in unique(comp_funcs_res$function_index)){
    ...
    print(summary.data.frame(comp_funcs_res[function_index == index, .(user_time, sys_time, elapsed_time, size, user_rate, sys_rate, elapsed_rate)]))
  }
}
...
#' @description S3 method for \code{lm} for Linear regression over time attributes by size for every function
#' @details Runs Linear regression over time attributes \code{user_time, sys_time, elapsed_time} as Y and \code{size} as X.
#' @param comp_funcs_res Object of class \code{comp.funcs.res.dt} - result of function \code{compare_functions()}
#' @return Returns a list where first level index corresponds to \code{function_index} and second level names \code{user_time, sys_time and elapsed_time}  have
#' lm result objects of respective time attributes \code{user_time, sys_time and elapsed_time} with \code{size} for the corresponding \code{function_index}
#' @export
...
#' @details S3 \code{plot} method for class \code{comp.funcs.res.dt}. Plots a line chart with function_index split by function_index with \code{size} in X axis
#' @param comp_funcs_res Object of class \code{comp.funcs.res.dt} - result of function \code{compare_functions()}
